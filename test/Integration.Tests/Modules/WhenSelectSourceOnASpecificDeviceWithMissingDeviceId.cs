...
{
    public class WhenSelectSourceOnASpecificDeviceWithMissingDeviceId : ContextBase
    {
        private readonly HttpResponseMessage response;

        private readonly ErrorResource result;

        public WhenSelectSourceOnASpecificDeviceWithMissingDeviceId()
        {
            var sourceId = Guid.NewGuid().ToString();

            var request = new
            {
                actionFields = new { source_id = sourceId },
