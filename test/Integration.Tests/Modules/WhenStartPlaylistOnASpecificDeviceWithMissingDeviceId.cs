namespace Linn.Api.Ifttt.Testing.Integration.Modules
{
    ...
    public class WhenStartPlaylistOnASpecificDeviceWithMissingDeviceId : ContextBase
    {
        private readonly HttpResponseMessage response;

        private readonly ErrorResource result;

        public WhenStartPlaylistOnASpecificDeviceWithMissingDeviceId()
        {
            var playlistId = Guid.NewGuid().ToString();

            var request = new
                              {
                                  actionFields = new { playlist_id = playlistId },
