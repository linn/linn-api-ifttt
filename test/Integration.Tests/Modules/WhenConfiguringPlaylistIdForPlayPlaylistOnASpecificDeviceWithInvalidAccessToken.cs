namespace Linn.Api.Ifttt.Testing.Integration.Modules
{
    ...
    public class WhenConfiguringPlaylistIdForPlayPlaylistOnASpecificDeviceWithInvalidAccessToken : ContextBase
    {
        private readonly HttpResponseMessage response;

        private readonly ErrorResource result;

        public WhenConfiguringPlaylistIdForPlayPlaylistOnASpecificDeviceWithInvalidAccessToken()
        {
            var request = new { };

            var content = new StringContent(JsonConvert.SerializeObject(request));

            this.LinnApiActions.GetPlaylistNames(Arg.Any<string>(), Arg.Any<CancellationToken>()).Throws(new LinnApiException(HttpStatusCode.Forbidden));
